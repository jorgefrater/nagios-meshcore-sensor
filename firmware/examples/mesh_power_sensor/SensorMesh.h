...
public:
  ...
  // Publica el voltaje de RED en la telemetría del nodo
  // (canal self, tipo LPP_VOLTAGE). La batería del nodo viaja como
  // LPP_ANALOG_INPUT en el mismo canal (ver SensorMesh.cpp).
  void publishLineVoltage(float volts) { telemetry.addVoltage(TELEM_CHANNEL_SELF, volts); }

  // mesh_power: envío periódico DIRECTO del heartbeat a los contactos con
  // permisos de alertas — 1 intento por ciclo, SIN depender del Trigger/ACK
  // (el mecanismo alertIf se bloquea ~20 min si no llega ACK).
  void sendPeriodicMessage(const char* text);
