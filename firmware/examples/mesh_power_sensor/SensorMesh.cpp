...
uint8_t SensorMesh::handleRequest(uint8_t perms, uint32_t sender_timestamp, uint8_t req_type, uint8_t* payload, size_t payload_len) {
  ...
  if (req_type == REQ_TYPE_GET_TELEMETRY_DATA) {  // allow all
    ...
    telemetry.reset();
    telemetry.addAnalogInput(TELEM_CHANNEL_SELF, (float)board.getBattMilliVolts() / 1000.0f);  // mesh_power: bateria como ANALOG; canal self VOLTAGE = red
...
// mesh_power: heartbeat periódico DIRECTO — 1 mensaje TXT por llamada, sin
// Trigger/ACK (sendAlert se bloquea ~20 min si el ACK no llega). Se manda a
// todos los contactos con permiso de alertas (LOW o HIGH).
void SensorMesh::sendPeriodicMessage(const char* text) {
  uint8_t data[MAX_PACKET_PAYLOAD];
  uint32_t ts = getRTCClock()->getCurrentTime();
  memcpy(data, &ts, 4);
  data[4] = (TXT_TYPE_PLAIN << 2) | 0;   // attempt 0, sin expected-ack
  int text_len = strlen(text);
  memcpy(&data[5], text, text_len);

  for (int i = 0; i < acl.getNumClients(); i++) {
    ClientInfo* c = acl.getClientByIdx(i);
    if ((c->permissions & (PERM_RECV_ALERTS_LO | PERM_RECV_ALERTS_HI)) == 0) continue;
